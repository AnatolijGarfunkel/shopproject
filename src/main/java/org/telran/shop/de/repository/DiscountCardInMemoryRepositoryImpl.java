...
@Component
public class DiscountCardInMemoryRepositoryImpl implements DiscountCardRepository {

    private Map<String, DiscountCard> storage = new HashMap<>();

    public DiscountCardInMemoryRepositoryImpl() {
        init();
    }
    ...
    public List<DiscountCard> filter(boolean expired) {
        return storage
                .values()
                .stream()
                .filter(card -> card.isExpired() == expired)
                .toList();
    }
}
